...
const products = [
  {
    ...
    "price": "$282",
    "picture": "http://placeimg.com/480/640/any",
    "color": "brown",
...
// utility functions for getting and setting cart items
const getCart = () => {
...
const addToCart = (i) => {
    ...
    let _cart = getCart()
    const index = _cart.findIndex( prod => prod._id == products[i]._id)
    if (index < 0){
        _cart.push(products[i])
        setCart(_cart)
    }
}
...
// function for drawing products, fitered or all
const drawProducts = () => {
    // decide what set of products to draw
    const _products = filteredProducts.length > 0 ? filteredProducts : products
    const product_cards = []
    // nullify product container for redraws
    ...
    for (pr of _products) {
        let tmp = `
                <img src="${pr.picture}" alt="Product Image">
                <div class="product-info">
                    <h3 class="product-title">${pr.name}</h3>
                    <p class="product-description">${pr.description.slice(0, 12)}...</p>
                    <div class="product-rating">
                        <span class="product-stars">★★★★☆</span>
                        <span class="product-reviews">(100)</span>
                    </div>
    ...
    // draw array of cards
    table.append(...product_cards)

    // add event listener for clicks for buy button
    const product_buttons = document.querySelectorAll('.product-button')
    for (let ind = 0; ind < product_buttons.length; ind++)  {
        product_buttons[ind].addEventListener(
            'click',
            addToCart.bind(null, ind)
        )
    }
}
...
document.getElementById('search').addEventListener('input', (event) => {
    filteredProducts = products.filter(
        pr => pr.name.includes(event.target.value)
    )
    // redraw after each search
    drawProducts()
})
